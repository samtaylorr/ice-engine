...
std::shared_ptr<LWindow> window = std::make_shared<LWindow>(Settings::SCREEN_WIDTH, Settings::SCREEN_HEIGHT);
...
//Rendered texture
std::unique_ptr<LFont> testText = std::make_unique<LFont>(window, *currentScene, "predev", gFont, SDL_Color{0,0,0,0});

//                                                                                                       Spritesheet                      Spritesheet instructions
std::shared_ptr<AnimatedSprite2D> animSprite = std::make_shared<AnimatedSprite2D>(window, *currentScene, "img/main_character_sprite.png", "img/main_character_sprite.json");
